...
class NordVPN:
    ...
    def update_title(self):
        while True:
            ...
            os.system(
                'title Fast NordVPN Checker - Checked: %s ^| Retries: %s ^| CPM: %s ^| Time Elapsed: %s ^| Threads: %s' % (
                self.data['checked'], self.data['retries'], self.data['cpm'], elapsed, (threading.active_count() - 2)))
            time.sleep(0.4)
