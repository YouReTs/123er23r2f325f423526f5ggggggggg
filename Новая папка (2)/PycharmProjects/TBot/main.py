# Чтобы создать бота, нужно дать ему название, адрес и получить токен — который будет идентифицировать бота для серверов
# Зайдем в Telegram под своим аккаунтом и откроем «отца всех ботов» https://t.me/BotFather
...
# Отправим бате‑боту команду /newbot, чтобы создать нового бота.
...
# Например, xakepbot или xakep_bot.
# Если адрес будет уже кем‑то занят, BotFather начнет извиняться и просить придумать что‑нибудь другое.


# Когда мы наконец найдем свободный и красивый адрес для нашего бота, в ответ получим сообщение, в котором после фразы
# Use this token to access the HTTP API будет написана строка из букв и цифр — это и есть необходимый нам токен.
# Сохраним ее где‑нибудь на своем компьютере, чтобы потом использовать в скрипте бота.


# Для взаимодействия с Telegram API есть несколько готовых модулей.
# Самый простой из них — Telebot. Чтобы установить его, набери
